sap.ui.define([
	...
	return BaseObject.extend("de.computerservice-wolf.iotui5.maintain.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias de.computerservice-wolf.iotui5.maintain.model.GroupSortState
		 */
		...
		/**
		 * Sorts by device, or by lastMeasurement
		 *
		...
		/**
		 * Groups by lastMeasurement, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function(sKey) {
			...
			if (sKey === "lastMeasurement") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "lastMeasurement");

				aSorters.push(
					new Sorter("lastMeasurement", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "device");
			}
